  ...
  switch (perfil) {
    ...
    case 'GERENTE':
      ...
      return MODULOS_SISTEMA.map((m) => {
        ...
        // configuracoes e stored_procedure fechados por padrão para gerente
    ...
    case 'AUDITOR':
      ...
      return MODULOS_SISTEMA.map((m) => {
        const comAcesso = ['dashboard', 'minhas_vendas', 'conferencia_vendedor', 'aprovacoes', 'repasses_admin', 'stored_procedure'].includes(m.id);
        return {
          modulo: m.id,
          acesso: comAcesso,
          inserir: false,
